...
12. [样式隔离](https://juejin.cn/post/7087239873354006564)

    1. 概念：一种确保不同组件、模块或Web应用组件的CSS样式互不干扰、不互相覆盖的技术手段。
    2. 常用手段：

       1. **BEM 命名规范**：手动命名（块\_\_元素--修饰符）降低冲突。
       2. ​**Scoped CSS (Vue/Blazor)** ​：编译时自动添加随机Hash值（如 `data-v-xxxx`）限制作用域。
       3. ​**CSS Modules (React/Vue)** ：构建工具（Webpack）将CSS类名作用域化为局部，自动映射类名。
       4. ​**Shadow DOM**：原生Web组件技术，提供独立的DOM树和CSS沙箱，实现真正的彻底隔离。
       5. **CSS-in-JS (React)** ：在JS中写CSS，动态生成独立类名。
    3. hash算法总结：
